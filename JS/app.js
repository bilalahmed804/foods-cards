...
let items = document.getElementById("foods");

for (const iterator of recipes) {
  //  console.log(iterator);
   items.innerHTML += `<div class="card"><img class="cardimge" src="${iterator.image}" alt="image">
   <h4>${iterator.name}</h4>
   <p>Calories<b>:</b> ${iterator.caloriesPerServing}</p>
   <p>Food ready in ${iterator.cookTimeMinutes} minutes</p>
   <p>Size<b>:</b> ${iterator.difficulty}</p>
   <p>Review<b>:</b> ${iterator.reviewCount}<p/>
   <button onclick="main()" class="button">Add to Card<button>
   </div>`
}
  function main(){
       Swal.fire({
...
function clickHandler(){
  ...
  let displyaContent = document.getElementById("foods")

  displyaContent.innerHTML = "";

  
  for (let i = 0; i < recipes.length; i++) {
    let element = recipes[i].name[0];
    // console.log(recipes[i].name);
    if(element.toLowerCase() == input.value.toLowerCase()){
      displyaContent.innerHTML += `<div class="card"><img class="cardimge" src="${recipes[i].image}" alt="image">
